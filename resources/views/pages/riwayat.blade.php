...
<section class="mx-auto max-w-4xl px-4 py-10">
  <!-- Child Info -->
  <div class="rounded-card p-6 border-2 mb-6" style="background:white; border-color:#09b1ab; box-shadow: 5px 5px 0 #d8f7f5">
    <div class="flex items-center gap-4">
      ...
      <div class="ml-auto">
        <span class="px-4 py-2 rounded-full text-sm font-bold
          {{ $enrollment->status === 'paid' ? 'bg-green-100 text-green-700' :
             ($enrollment->status === 'confirmed' ? 'bg-yellow-100 text-yellow-700' :
  ...
  <!-- Payment History -->
  <div class="rounded-card p-6 border-2" style="background:white; border-color:#ffd900; box-shadow: 5px 5px 0 #ffd900">
    <h3 class="font-display text-lg font-black text-ink mb-4">📄 Riwayat Pembayaran</h3>

    @forelse($payments as $payment)
    <div class="flex items-center justify-between p-4 {{ $loop->first ? '' : 'border-t border-gray-100' }} hover:bg-gray-50 transition-colors">
      <div class="flex items-center gap-4">
        <div class="w-12 h-12 rounded-full {{ $payment->status === 'paid' ? 'bg-green-100' : 'bg-yellow-100' }} flex items-center justify-center">
        ...
        <div>
          ...
          <p class="text-xs font-semibold" style="color:#5c5555">
            ...
            @if($payment->invoice) • Invoice: {{ $payment->invoice->invoice_number }} @endif
          </p>
        </div>
      </div>
      <div class="text-right">
        <p class="font-black text-ink">Rp {{ number_format($payment->amount, 0, ',', '.') }}</p>
        <span class="text-xs font-bold {{ $payment->status === 'paid' ? 'text-green-600' : 'text-yellow-600' }}">
    ...
    @if($payment->invoice)
    <div class="px-4 pb-4">
      <a href="{{ route('public.invoice.show', $payment->invoice->slug) }}" target="_blank" class="text-sm text-teal font-bold hover:underline">
        📄 Lihat Invoice →
      </a>
    </div>
    @endif
