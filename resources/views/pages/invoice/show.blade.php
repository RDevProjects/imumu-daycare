...
<section class="px-4 py-10 md:py-14">
  <div class="mx-auto max-w-2xl">
    ...
    <!-- Invoice Card -->
    <div class="rounded-card p-8 border-2" style="background:white; border-color:#09b1ab; box-shadow: 5px 5px 0 #d8f7f5">
        ...
        <tbody>
          <tr class="border-b border-gray-100">
            <td class="py-4">
              ...
              <p class="text-sm" style="color:#5c5555">Tipe: {{ ucfirst($invoice->enrollment->package->type) }}</p>
              <p class="text-sm" style="color:#5c5555">Metode: {{ $invoice->payment->payment_method === 'cash' ? 'Cash' : 'Transfer' }}</p>
            </td>
            <td class="py-4 text-right font-bold text-ink">Rp {{ number_format($invoice->total_amount, 0, ',', '.') }}</td>
          </tr>
        </tbody>
        <tfoot>
          <tr>
      ...
      <div class="text-center pt-6 border-t border-gray-100">
        <p class="text-sm font-bold" style="color:#5c5555">{{ \App\Models\Setting::get('invoice_header_text', 'Terima kasih atas kepercayaan Anda kepada IMUMU Daycare.') }}</p>
        <p class="text-xs mt-4" style="color:#999">{{ \App\Models\Setting::get('invoice_footer_text', 'Invoice ini digenerate otomatis oleh sistem.') }}</p>
      </div>
    </div>
    ...
    <div class="mt-6 text-center">
      ...
      <a href="https://wa.me/{{ \App\Models\Setting::get('daycare_wa', '6285877748008') }}" target="_blank" class="inline-flex items-center gap-2 mt-2 px-6 py-3 rounded-full font-bold text-white" style="background:#25d366">
        💬 Chat WhatsApp
      </a>
    </div>
  </div>
...
<style>
@media print {
  header, footer, nav, .btn, .no-print, button { display: none !important; }
  body { margin: 0; padding: 10px; background: white !important; }
  .rounded-card { box-shadow: none !important; border: 2px solid #09b1ab !important; }
