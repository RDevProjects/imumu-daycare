...
<div x-data="{
  ...
  <div class="bg-white dark:bg-imumu-dark-card dark:border-imumu-dark-border rounded-xl shadow-sm p-1.5 mb-6">
    <div class="flex gap-1 overflow-x-auto">
      ...
      <button @click="activeSection = 'rekening'" :class="activeSection === 'rekening' ? 'bg-imumu-purple text-white' : 'text-gray-500 dark:text-gray-400 hover:bg-gray-100 dark:hover:bg-imumu-dark-surface'" class="flex-1 py-2.5 px-4 rounded-lg font-semibold text-sm transition-all whitespace-nowrap cursor-pointer">Rekening Bank</button>
    ...
    <div class="card">
      ...
      <form action="{{ route('dashboard.pengaturan.profile') }}" method="POST">
        ...
        <div class="flex items-center gap-5 mb-6">
          ...
          <div>
            <h4 class="text-lg font-bold text-gray-700 dark:text-imumu-dark-text">{{ Auth::user()->name }}</h4>
        ...
        <div class="space-y-4">
          <div>
            ...
            <input type="text" name="name" value="{{ old('name', Auth::user()->name) }}" class="input-field @error('name') border-red-500 @enderror">
            @error('name') <p class="text-red-500 text-xs mt-1">{{ $message }}</p> @enderror
          </div>
          <div>
            <label class="block text-sm font-semibold text-gray-600 dark:text-gray-300 mb-1">Email</label>
            <input type="email" name="email" value="{{ old('email', Auth::user()->email) }}" class="input-field @error('email') border-red-500 @enderror">
            @error('email') <p class="text-red-500 text-xs mt-1">{{ $message }}</p> @enderror
          </div>
          <div>
            <label class="block text-sm font-semibold text-gray-600 dark:text-gray-300 mb-1">Telepon</label>
            <input type="tel" name="phone" value="{{ old('phone', Auth::user()->phone ?? '') }}" class="input-field">
          </div>
          <div class="pt-4 border-t border-gray-100 dark:border-imumu-dark-border">
            ...
            <div class="space-y-3">
              <div>
                <input type="password" name="current_password" placeholder="Password saat ini" class="input-field @error('current_password') border-red-500 @enderror">
                @error('current_password') <p class="text-red-500 text-xs mt-1">{{ $message }}</p> @enderror
              </div>
              <input type="password" name="new_password" placeholder="Password baru" class="input-field @error('new_password') border-red-500 @enderror">
              @error('new_password') <p class="text-red-500 text-xs mt-1">{{ $message }}</p> @enderror
              <input type="password" name="new_password_confirmation" placeholder="Konfirmasi password baru" class="input-field">
            </div>
          </div>
  ...
  <div x-show="activeSection === 'daycare'" class="max-w-2xl space-y-6">
    <div class="card">
      ...
      <form action="{{ route('dashboard.pengaturan.update') }}" method="POST">
        ...
        <div class="space-y-4">
          <div>
            <label class="block text-sm font-semibold text-gray-600 dark:text-gray-300 mb-1">Nama Daycare</label>
            <input type="text" name="daycare_name" value="{{ old('daycare_name', $settings['daycare_name'] ?? 'IMUMU Daycare') }}" class="input-field">
          </div>
          <div>
          ...
          <div class="pt-4 border-t border-gray-100 dark:border-imumu-dark-border">
            ...
            <div class="grid grid-cols-2 gap-4">
              <div>
                ...
                <input type="time" name="daycare_open_time" value="{{ old('daycare_open_time', $settings['daycare_open_time'] ?? '08:00') }}" class="input-field">
              </div>
              <div>
                <label class="block text-sm text-gray-500 dark:text-gray-400 mb-1">Tutup</label>
                <input type="time" name="daycare_close_time" value="{{ old('daycare_close_time', $settings['daycare_close_time'] ?? '16:00') }}" class="input-field">
              </div>
            </div>
          </div>
          <div class="flex gap-3 pt-4">
  ...
  <div x-show="activeSection === 'tarif'" x-data="{
    ...
    <div class="card">
      <div class="flex items-center justify-between mb-4">
        <div>
          <h3 class="text-lg font-bold text-gray-700 dark:text-imumu-dark-text font-poppins">Paket Penitipan</h3>
      ...
      <div class="space-y-3">
        ...
        <form action="{{ route('dashboard.packages.update', $pkg) }}" method="POST" class="flex items-center gap-4 p-4 bg-gray-50 dark:bg-imumu-dark-surface rounded-xl">
          @csrf
          @method('PATCH')
          <div class="flex-1 grid grid-cols-2 sm:grid-cols-4 gap-3">
            <input type="text" name="name" value="{{ $pkg->name }}" class="input-field text-sm" placeholder="Nama Paket">
      ...
      {{-- Tambah Paket Baru --}}
      <form action="{{ route('dashboard.packages.store') }}" method="POST" class="mt-4 pt-4 border-t border-gray-100 dark:border-imumu-dark-border">
        @csrf
        <div class="flex items-center gap-4 p-4 bg-green-50 dark:bg-green-900/10 rounded-xl border border-green-200 dark:border-green-800">
          <div class="flex-1 grid grid-cols-2 sm:grid-cols-4 gap-3">
  ...
  <div x-show="activeSection === 'rekening'" class="max-w-3xl space-y-6">
    <div class="card">
      ...
      <p class="text-sm text-gray-400 dark:text-gray-400 mb-6">Rekening untuk pembayaran transfer. Ditampilkan saat konfirmasi pendaftaran.</p>

      <div class="space-y-3">
        @forelse($banks as $bank)
        <form action="{{ route('dashboard.banks.update', $bank) }}" method="POST" class="flex items-center gap-4 p-4 {{ $bank->is_active ? 'bg-gray-50 dark:bg-imumu-dark-surface' : 'bg-gray-100 dark:bg-gray-800 opacity-60' }} rounded-xl">
          @csrf
          @method('PATCH')
          <div class="flex-1 grid grid-cols-1 sm:grid-cols-4 gap-3">
            <input type="text" name="bank_name" value="{{ $bank->bank_name }}" class="input-field text-sm" placeholder="Nama Bank">
            <input type="text" name="account_number" value="{{ $bank->account_number }}" class="input-field text-sm" placeholder="No Rekening">
            <input type="text" name="account_name" value="{{ $bank->account_name }}" class="input-field text-sm" placeholder="Nama Pemilik">
            <div class="flex items-center gap-2">
              <button type="submit" class="btn-primary text-sm py-2 flex-1">Simpan</button>
              <button type="button" onclick="fetch('{{ route('dashboard.banks.toggle', $bank) }}', {method: 'POST', headers: {'X-CSRF-TOKEN': '{{ csrf_token() }}'}}).then(() => location.reload())" class="px-2 py-2 text-xs rounded-lg {{ $bank->is_active ? 'bg-green-100 text-green-600' : 'bg-gray-200 text-gray-500' }}" title="{{ $bank->is_active ? 'Nonaktifkan' : 'Aktifkan' }}">
                {{ $bank->is_active ? '✓' : '✗' }}
              </button>
              <button type="button" onclick="if(confirm('Hapus rekening ini?')) fetch('{{ route('dashboard.banks.destroy', $bank) }}', {method: 'DELETE', headers: {'X-CSRF-TOKEN': '{{ csrf_token() }}'}}).then(() => location.reload())" class="px-2 py-2 bg-red-100 text-red-600 rounded-lg hover:bg-red-200 text-sm">
                <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6m1-10V4a1 1 0 00-1-1h-4a1 1 0 00-1 1v3M4 7h16"/></svg>
              </button>
            </div>
          </div>
        ...
        <div class="text-center py-8 text-gray-400">
          <svg class="w-12 h-12 mx-auto mb-3 opacity-50" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M3 10h18M7 15h1m4 0h1m-7 4h12a3 3 0 003-3V8a3 3 0 00-3-3H6a3 3 0 00-3 3v8a3 3 0 003 3z"/></svg>
      ...
      <form action="{{ route('dashboard.banks.store') }}" method="POST" class="mt-4 pt-4 border-t border-gray-100 dark:border-imumu-dark-border">
        @csrf
        <div class="flex items-center gap-4 p-4 bg-green-50 dark:bg-green-900/10 rounded-xl border border-green-200 dark:border-green-800">
          <div class="flex-1 grid grid-cols-1 sm:grid-cols-4 gap-3">
            <input type="text" name="bank_name" class="input-field text-sm" placeholder="Nama Bank (BCA, Mandiri, dll)" required>
            <input type="text" name="account_number" class="input-field text-sm" placeholder="No Rekening" required>
            <input type="text" name="account_name" class="input-field text-sm" placeholder="Nama Pemilik Rekening" required>
  ...
  <div x-show="activeSection === 'notifikasi'" class="max-w-2xl space-y-6">
    <div class="card">
      <h3 class="text-lg font-bold text-gray-700 dark:text-imumu-dark-text mb-6 font-poppins">Template WhatsApp</h3>
      <form action="{{ route('dashboard.pengaturan.update') }}" method="POST">
        @csrf
        <div class="space-y-4">
          <div>
            <label class="block text-sm font-semibold text-gray-600 dark:text-gray-300 mb-1">Template Konfirmasi Pendaftaran</label>
            <p class="text-xs text-gray-400 dark:text-gray-400 mb-2">Placeholder: {parent_name}, {child_name}, {package_name}, {amount}, {bank_info}</p>
