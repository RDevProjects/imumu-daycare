  ...
  <div class="bg-white dark:bg-imumu-dark-card rounded-xl shadow-sm p-4">
    <form method="GET" class="flex flex-wrap gap-3">
      <div class="flex-1 min-w-[200px]">
        <input type="text" name="search" value="{{ request('search') }}" placeholder="🔍 Cari nama anak / ortu / no registrasi..." class="input-field w-full">
      </div>
    ...
    <div class="stat-card border-l-4 border-yellow-500">
      <div class="flex items-center justify-between">
        <div>
          ...
          <p class="text-2xl font-bold text-gray-700 dark:text-imumu-dark-text mt-1">{{ \App\Models\Enrollment::pending()->count() }}</p>
        </div>
    ...
    <div class="stat-card border-l-4 border-blue-500">
      <div class="flex items-center justify-between">
        <div>
          ...
          <p class="text-2xl font-bold text-gray-700 dark:text-imumu-dark-text mt-1">{{ \App\Models\Enrollment::confirmed()->count() }}</p>
        </div>
    ...
    <div class="stat-card border-l-4 border-green-500">
      <div class="flex items-center justify-between">
        <div>
          ...
          <p class="text-2xl font-bold text-gray-700 dark:text-imumu-dark-text mt-1">{{ \App\Models\Enrollment::paid()->count() }}</p>
        </div>
    ...
    <div class="stat-card border-l-4 border-imumu-pink-dark">
      <div class="flex items-center justify-between">
        <div>
          ...
          <p class="text-2xl font-bold text-gray-700 dark:text-imumu-dark-text mt-1">{{ \App\Models\Enrollment::count() }}</p>
        </div>
    ...
    <table class="w-full text-sm">
      ...
      <tbody>
        ...
        <tr class="border-b border-gray-50 dark:border-imumu-dark-border hover:bg-gray-50 dark:hover:bg-imumu-dark-surface transition-colors">
          <td class="py-3 px-4">
            <span class="font-mono text-xs font-bold text-gray-600 dark:text-gray-300">{{ $enr->registration_number }}</span>
          </td>
          <td class="py-3 px-4">
          ...
          <td class="py-3 px-4">
            ...
            <p class="text-xs text-gray-400">Rp {{ number_format($enr->package->price, 0, ',', '.') }}</p>
          </td>
          <td class="py-3 px-4">
            <span class="text-xs {{ $enr->payment_method === 'cash' ? 'text-green-600' : 'text-blue-600' }}">
          ...
          <td class="py-3 px-4">
            <div class="flex gap-2 flex-wrap">
              ...
              {{-- Konfirmasi Form --}}
              <form action="{{ route('dashboard.pendaftaran.confirm', $enr) }}" method="POST" class="inline" onsubmit="return confirm('Konfirmasi pendaftaran {{ $enr->child_name }}?')">
                @csrf
              ...
              <a href="{{ $waUrl }}" target="_blank" class="px-3 py-1.5 bg-green-100 text-green-700 text-xs font-semibold rounded-lg hover:bg-green-200 flex items-center gap-1 no-underline">
                💬 Chat WA
              </a>
              @endif

              @if($enr->status === 'confirmed')
              <a href="{{ route('dashboard.pembayaran') }}" class="px-3 py-1.5 bg-blue-100 text-blue-600 text-xs font-semibold rounded-lg hover:bg-blue-200 no-underline">
                💰 Bayar
              </a>
              @endif

              <a href="{{ $enr->history_url }}" target="_blank" class="px-3 py-1.5 bg-gray-100 text-gray-600 text-xs font-semibold rounded-lg hover:bg-gray-200 no-underline">
                🔗 Riwayat
              </a>
            </div>
          </td>
