...
<div class="max-w-3xl mx-auto space-y-6">
  ...
  <div class="card">
    ...
    <div class="grid grid-cols-2 gap-4">
      ...
      <div class="bg-gray-50 dark:bg-imumu-dark-surface rounded-xl p-4">
        ...
        <p class="font-bold text-gray-700 dark:text-imumu-dark-text">{{ $payment->enrollment->package->label }}</p>
      </div>
      <div class="bg-gray-50 dark:bg-imumu-dark-surface rounded-xl p-4">
        <p class="text-sm text-gray-400">Nominal</p>
        <p class="font-bold text-2xl" style="color: #09b1ab">Rp {{ number_format($payment->amount, 0, ',', '.') }}</p>
      </div>
      <div class="bg-gray-50 dark:bg-imumu-dark-surface rounded-xl p-4">
        <p class="text-sm text-gray-400">Metode</p>
        <p class="font-bold text-gray-700 dark:text-imumu-dark-text">{{ $payment->payment_method === 'cash' ? '💵 Cash' : '🏦 Transfer' }}</p>
      </div>
      <div class="bg-gray-50 dark:bg-imumu-dark-surface rounded-xl p-4">
  ...
  <div class="card">
    ...
    <form action="{{ route('dashboard.pembayaran.mark-paid', $payment) }}" method="POST" enctype="multipart/form-data" class="space-y-4">
      @csrf

      {{-- Upload Bukti Transfer (arsip admin) --}}
      <div>
        <label class="block text-sm font-semibold text-gray-600 dark:text-gray-300 mb-2">Upload Bukti Transfer (arsip)</label>
        <div class="flex items-center gap-4">
          <input type="file" name="bukti_transfer" accept="image/*" class="input-field" id="buktiInput" onchange="previewImage(this)">
        </div>
        <div id="imagePreview" class="mt-3 hidden">
          <img src="" alt="Preview" class="max-w-xs rounded-xl border-2 border-gray-200 dark:border-imumu-dark-border">
        </div>
        <p class="text-xs text-gray-400 mt-1">Screenshot bukti transfer dari WhatsApp (opsional, untuk arsip)</p>
      </div>

      <div>
        <label class="block text-sm font-semibold text-gray-600 dark:text-gray-300 mb-1">Tanggal Bayar</label>
        <input type="date" name="payment_date" value="{{ now()->format('Y-m-d') }}" class="input-field w-48">
      </div>
  ...
  <div class="card text-center py-8">
    ...
    <p class="text-xs text-gray-400 mt-2">Diverifikasi pada: {{ $payment->verified_at->format('d M Y H:i') }}</p>
    @endif
    <a href="{{ route('dashboard.pembayaran') }}" class="btn-primary mt-4 inline-block">Kembali ke Pembayaran</a>
  </div>
  @endif
</div>
