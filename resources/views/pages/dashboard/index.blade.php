...
<div class="bg-imumu-pink dark:bg-imumu-dark-pink rounded-2xl p-6 md:p-8 mb-8 shadow-sm relative overflow-hidden">
  ...
  <div class="relative z-10">
    ...
    <p class="text-white/80 mb-4">Hari ini ada <strong class="text-white">{{ $anakAktif ?? 0 }} anak</strong> yang aktif. Semangat!</p>
    <div class="flex gap-3">
      <a href="{{ route('dashboard.pendaftaran') }}" class="bg-white/90 text-imumu-pink-dark font-semibold px-5 py-2 rounded-full hover:bg-white transition-colors shadow-sm text-sm">Lihat Pendaftaran</a>
      <a href="{{ route('dashboard.profile-anak') }}" class="bg-white/20 text-white font-semibold px-5 py-2 rounded-full hover:bg-white/30 transition-colors border border-white/30 text-sm">Data Anak</a>
    </div>
  </div>
</div>
  ...
  <div class="stat-card border-l-4 border-yellow-500">
    <div class="flex items-center justify-between">
      <div>
        ...
        <p class="text-2xl font-bold text-gray-700 dark:text-imumu-dark-text mt-1">{{ $enrollmentsPending ?? 0 }}</p>
  ...
  <div class="stat-card border-l-4 border-imumu-blue">
    <div class="flex items-center justify-between">
      <div>
        ...
        <p class="text-2xl font-bold text-gray-700 dark:text-imumu-dark-text mt-1">{{ $paymentsPending ?? 0 }}</p>
  ...
  <div class="stat-card border-l-4 border-green-500">
    <div class="flex items-center justify-between">
      <div>
        ...
        <p class="text-2xl font-bold text-gray-700 dark:text-imumu-dark-text mt-1">Rp {{ number_format($monthlyRevenue ?? 0, 0, ',', '.') }}</p>
...
<div class="grid grid-cols-1 lg:grid-cols-2 gap-6 mb-8">
  ...
  <div class="card">
    <div class="flex items-center justify-between mb-4">
      ...
      <a href="{{ route('dashboard.pendaftaran') }}" class="text-sm text-imumu-pink-dark dark:text-imumu-dark-pink font-semibold hover:underline">Lihat Semua →</a>
    </div>
    <div class="space-y-3">
  ...
  <div class="card">
    <div class="flex items-center justify-between mb-4">
      ...
      <a href="{{ route('dashboard.pembayaran') }}" class="text-sm text-imumu-pink-dark dark:text-imumu-dark-pink font-semibold hover:underline">Lihat Semua →</a>
    </div>
    <div class="space-y-3">
      @forelse($recentPayments as $payment)
      <div class="flex items-center justify-between p-3 bg-gray-50 dark:bg-imumu-dark-surface rounded-xl">
        <div>
          <p class="font-semibold text-gray-700 dark:text-imumu-dark-text">{{ $payment->enrollment->child_name }}</p>
          <p class="text-xs text-gray-400">{{ $payment->payment_method === 'cash' ? '💵 Cash' : '🏦 Transfer' }} • Rp {{ number_format($payment->amount, 0, ',', '.') }}</p>
        </div>
        <span class="px-2 py-1 rounded-full text-xs font-bold
