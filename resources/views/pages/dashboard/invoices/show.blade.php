            ...
            <div class="flex justify-between items-start mb-8">
                ...
                <div class="text-right">
                    ...
                    <p class="font-bold text-gray-700 dark:text-imumu-dark-text">
                        {{ $invoice->issued_date->format('d F Y') }}</p>
            ...
            <table class="w-full mb-8">
                ...
                <tbody>
                    <tr class="border-b border-gray-100 dark:border-imumu-dark-border">
                        <td class="py-4">
                            <p class="font-bold text-gray-700 dark:text-imumu-dark-text">
                                {{ $invoice->enrollment->package->name }}</p>
                            <p class="text-sm text-gray-400">Tipe: {{ ucfirst($invoice->enrollment->package->type) }}</p>
                            <p class="text-sm text-gray-400">Metode:
                                {{ $invoice->payment->payment_method === 'cash' ? 'Cash' : 'Transfer' }}</p>
                        </td>
                        <td class="py-4 text-right font-bold text-gray-700 dark:text-imumu-dark-text">Rp
