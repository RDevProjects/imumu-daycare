...
<header class="sticky top-0 z-40 bg-white/98 backdrop-blur-sm"
    style="box-shadow: 0 4px 0 #ffd900, 0 6px 20px rgba(47,43,43,0.08)">
    <nav class="mx-auto flex max-w-7xl items-center justify-between px-4 py-3 md:px-8" aria-label="Navigasi utama">
        <a href="{{ route('home') }}" class="flex items-center gap-3 no-underline group"
            aria-label="IMUMU Daycare beranda">
        ...
        <ul class="hidden md:flex items-center gap-2" role="list">
            ...
            <li><a href="{{ route('programs') }}" class="nav-link {{ $isActive('programs') }}">Program &amp;
                    Fasilitas</a></li>
            ...
            @auth
                ...
                <form action="{{ route('logout') }}" method="POST" class="inline">
                    ...
                    <button type="submit" class="btn btn-ghost btn-sm"
                        style="border-color:#e83f7d; color:#e83f7d; box-shadow: 3px 3px 0 rgba(232,63,125,0.2)">Logout</button>
                </form>
            @else
                @if ($waNumber)
                    <a href="https://wa.me/{{ $waNumber }}?text={{ $waMessage }}" target="_blank" rel="noopener"
                        class="btn btn-secondary btn-sm">💬 {{ $waDisplayName }}</a>
                @endif
        ...
        <button id="mobile-menu-btn"
            class="flex md:hidden items-center justify-center w-11 h-11 rounded-full font-bold text-white"
            style="background-color:#09b1ab" aria-label="Buka menu" aria-expanded="false">
    ...
    <!-- Mobile menu panel -->
    @php
        $isActiveMobile = fn($page) => $activePage === $page
            ? 'font-semibold text-teal" style="background-color:#d8f7f5'
    ...
    <div id="mobile-menu" class="hidden md:hidden bg-white border-t-2 border-border-default px-4 py-4 space-y-2">
        ...
        <a href="{{ route('programs') }}"
            class="block px-4 py-3 rounded-soft {{ $isActiveMobile('programs') }}">Program &amp; Fasilitas</a>
        <a href="{{ route('daftar') }}" class="block px-4 py-3 rounded-soft {{ $isActiveMobile('daftar') }}">Daftar
            Sekarang</a>
        @if ($waNumber)
            <a href="https://wa.me/{{ $waNumber }}?text={{ $waMessage }}" target="_blank" rel="noopener"
                class="btn btn-secondary w-full mt-2">💬 Hubungi {{ $waDisplayName }}</a>
        @endif
    </div>
</header>
