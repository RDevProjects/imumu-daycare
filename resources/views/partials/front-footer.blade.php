...
@endphp
        <div class="mt-4 flex gap-3">
          ...
          @if($waNumber)
          <a href="https://wa.me/{{ $waNumber }}?text={{ $waMessage }}" target="_blank" rel="noopener" aria-label="WhatsApp IMUMU Daycare" class="flex h-10 w-10 items-center justify-center rounded-full transition-all hover:-translate-y-0.5" style="background:#18a058; color:white">
      ...
      <div>
        ...
        <ul class="space-y-3 text-sm" style="color:rgba(255,255,255,0.7)" role="list">
          ...
          <li class="flex items-start gap-2">
            <span>📱</span>
            <a href="https://wa.me/{{ $waNumber }}?text={{ $waMessage }}" class="hover:text-white transition-colors no-underline">{{ $waDisplayNumber }}<br/><span class="text-xs" style="color:#09b1ab">({{ $waDisplayName }})</span></a>
          </li>
          @endif
          <li class="flex items-start gap-2">
            <span>📸</span>
          ...
          <li class="flex items-start gap-2">
            <span>🕐</span>
            <span>Senin–Sabtu<br/>{{ $openFormatted }}–{{ $closeFormatted }} WIB<br/><span class="text-xs italic">Minggu &amp; Merah: Tutup</span></span>
          </li>
        </ul>
      </div>
      ...
      <div>
        ...
        <a href="https://maps.google.com?q={{ urlencode($daycareAddress) }}" target="_blank" rel="noopener" class="inline-flex items-center gap-1 mt-3 text-xs font-bold no-underline hover:text-white transition-colors" style="color:#09b1ab">
          Buka di Google Maps →
        </a>
      </div>
    </div>
